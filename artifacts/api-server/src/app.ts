...
async function runV2Migration(): Promise<void> {
  const steps = [
    ...
    `CREATE TABLE IF NOT EXISTS byaj_accounts (
      ...
      status VARCHAR(20) NOT NULL DEFAULT 'ACTIVE', next_due_date DATE,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )`,
    ...
    `CREATE TABLE IF NOT EXISTS loan_accounts (
      ...
      stage VARCHAR(20) NOT NULL DEFAULT 'APPLIED', notes TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )`,
    ...
    // 5. Universal payment ledger
    `CREATE TABLE IF NOT EXISTS payment_ledger (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      customer_id TEXT,
      module VARCHAR(30) NOT NULL, source_id UUID, source_table VARCHAR(60),
      amount NUMERIC(12,2) NOT NULL, payment_mode VARCHAR(20) NOT NULL DEFAULT 'CASH',
  ...
  for (const sql of steps) {
    try {
      ...
      // Non-fatal: might already exist or have enum conflict
      fail++;
      logger.debug({ sql: sql.slice(0, 60), err: err.message }, "V2 migration step warning");
    }
  }
  logger.info({ ok, fail }, "V2 schema migration complete");
}
