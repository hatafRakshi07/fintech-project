  ...
  try {
    ...
    const result = await Promise.all(
      rows.map(async (row: (typeof rows)[number]) => {
        ...
        if (row.l.winnerId) {
          const [cust] = await db.select().from(customersTable).where(eq(customersTable.id, row.l.winnerId)).catch(() => []);
...
router.post("/lotteries", async (req, res): Promise<void> => {
  try {
    ...
    const [lottery] = await db
      ...
      .returning();
    res.status(201).json({ ...lottery, committeeName: null, winnerName: null, winnerToken: null, prizeAmount: lottery.prizeAmount ? parseFloat(lottery.prizeAmount) : null, createdAt: safeIso(lottery.createdAt) });
  } catch (err: any) {
...
router.get("/lotteries/:id", async (req, res): Promise<void> => {
  try {
    ...
    res.json({ ...row.l, committeeName: row.committeeName, winnerName: null, winnerToken: null, prizeAmount: row.l.prizeAmount ? parseFloat(row.l.prizeAmount) : null, createdAt: safeIso(row.l.createdAt) });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
router.patch("/lotteries/:id", async (req, res): Promise<void> => {
  try {
    ...
    res.json({ ...lottery, committeeName: null, winnerName: null, winnerToken: null, prizeAmount: lottery.prizeAmount ? parseFloat(lottery.prizeAmount) : null, cashTaken: lottery.cashTaken ? parseFloat(lottery.cashTaken) : null, createdAt: safeIso(lottery.createdAt) });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
router.post("/lotteries/:id/draw", async (req, res): Promise<void> => {
  try {
    ...
    const [cust] = await db.select().from(customersTable).where(eq(customersTable.id, winner.customerId)).catch(() => []);
    ...
    res.json({
      ...
      createdAt: safeIso(updated.createdAt),
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.get("/lotteries/:id/members", async (req, res): Promise<void> => {
  try {
    const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);
    const [lottery] = await db.select({ committeeId: lotteriesTable.committeeId }).from(lotteriesTable).where(eq(lotteriesTable.id, id)).catch(() => []);
    if (!lottery) { res.status(404).json({ error: "Lottery not found" }); return; }
