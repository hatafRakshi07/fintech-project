...
router.get("/branches", async (req, res): Promise<void> => {
  try {
    ...
    const rows = await (query as any).orderBy(branchesTable.name);
...
router.post("/branches", async (req, res): Promise<void> => {
  try {
    ...
    const [branch] = await db
      ...
      .returning();
    res.status(201).json({ ...branch, totalCustomers: 0, totalCollectors: 0, createdAt: safeIso(branch.createdAt) });
  } catch (err: any) {
...
router.get("/branches/:id", async (req, res): Promise<void> => {
  try {
    ...
    res.json({
      ...
      createdAt: safeIso(branch.createdAt),
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.patch("/branches/:id", async (req, res): Promise<void> => {
  try {
    ...
    const [branch] = await db
      ...
      .returning();
    if (!branch) { res.status(404).json({ error: "Branch not found" }); return; }
    res.json({ ...branch, totalCustomers: 0, totalCollectors: 0, createdAt: safeIso(branch.createdAt) });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
