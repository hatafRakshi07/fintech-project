...
router.get("/committees", async (req, res): Promise<void> => {
  try {
    ...
    const result = await Promise.all(
      rows.map(async (row) => {
        ...
        return {
          ...
          installmentAmount: parseFloat(row.com.installmentAmount || "0"),
          currentMembers: memberCount?.count ?? 0,
          totalCollected: parseFloat(collected?.sum ?? "0"),
          createdAt: safeIso(row.com.createdAt),
        };
      })
    );
    ...
    sendErrorListResponse(res, err?.message || "Failed to fetch committees");
  }
});
...
router.post("/committees", async (req, res): Promise<void> => {
  try {
    ...
    const [com] = await db
      ...
      .returning();
    res.status(201).json({ ...com, installmentAmount: parseFloat(com.installmentAmount), currentMembers: 0, totalCollected: 0, createdAt: safeIso(com.createdAt) });
  } catch (err: any) {
...
router.patch("/committees/:id", async (req, res): Promise<void> => {
  try {
    ...
    res.json({ ...com, installmentAmount: parseFloat(com.installmentAmount), currentMembers: 0, totalCollected: 0, createdAt: safeIso(com.createdAt) });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
router.post("/committees/:id/members", async (req, res): Promise<void> => {
  try {
    ...
    res.status(201).json({
      ...
      joinedAt: safeIso(member.joinedAt),
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
