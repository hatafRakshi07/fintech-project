    ...
    const result = await pool.query(`
      ...
      LEFT JOIN (
        SELECT loan_id,
          ...
          SUM(amount_deposited) FILTER(WHERE payment_date::date >= DATE_TRUNC('week', CURRENT_DATE)::date) AS week_collected,
          SUM(amount_deposited) FILTER(WHERE payment_date::date >= DATE_TRUNC('month', CURRENT_DATE)::date) AS month_collected
