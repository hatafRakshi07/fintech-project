...
export default function CollectorHistoryPage() {
  ...
  const { data: rawHistory, isLoading, error } = useQuery<any[]>({
    queryKey: ["collector-history", collectorId],
    queryFn: () => api.get(`/collections?collectorId=${collectorId}`),
    enabled: !!collectorId,
  });
  ...
  const handleExport = () => {
    if (historyList.length > 0) {
      exportToExcel({ History: historyList }, `collector_${collectorId}_history`);
    }
  };

  return (
    <>
      <Header title="Collector Full History" back={true} />
                ...
                {historyList.map((h) => (
                  <tr key={h.id} className="border-t border-border dark:border-slate-700/30">
                    <td className="px-3 py-1 text-xs">
                      {fmt.date(h.collectedAt)} {new Date(h.collectedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </td>
                    <td className="px-3 py-1 text-xs truncate max-w-xs">{h.customerName || `Customer #${h.customerId}`}</td>
