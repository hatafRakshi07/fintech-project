...
const STATUS_CLASSES: Record<string, string> = {
  ...
  in_progress: "bg-amber-500/20 text-amber-600 border border-amber-500/30",
  ...
  written_off: "bg-slate-200 dark:bg-slate-800/40 text-slate-500 border border-border dark:border-slate-700/30",
};

const PRIORITY_CLASSES: Record<string, string> = {
  low: "bg-slate-200 dark:bg-slate-800/40 text-slate-500 border border-border dark:border-slate-700/30",
...
export default function RecoveryPage() {
  ...
  return (
    <>
      ...
      <div className="p-4 space-y-3 max-w-lg mx-auto float-up">
        {/* Filter pills */}
            ...
            <button
              ...
              className={`shrink-0 px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${
                statusFilter === f.value
                  ...
                  : "bg-slate-200 dark:bg-slate-800/40 text-slate-500 dark:text-slate-400 border-border dark:border-slate-700/50 hover:text-foreground"
              }`}
            >
              {f.label}
            </button>
        ...
        {isLoading ? (
          <div className="space-y-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="glass-card rounded-2xl p-4 h-20 animate-pulse bg-card" />
            ))}
          </div>
        ) : filteredTasks.length === 0 ? (
          <div className="text-center py-16 glass-card rounded-2xl">
            <CheckCircle2 size={40} className="mx-auto mb-2 opacity-30 text-amber-500" />
            <p className="text-sm font-medium text-slate-500">No recovery tasks found</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {filteredTasks.map((task) => (
              <div key={task.id} className="glass-card rounded-2xl hover:border-amber-500/25 transition-all overflow-hidden bg-card">
                {/* Task header */}
                <button
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <p className="font-bold text-foreground dark:text-white text-sm">{task.customerName ?? "Unknown"}</p>
                      <span className={`text-[9px] px-2 py-0.2 rounded-full font-bold uppercase tracking-wider ${PRIORITY_CLASSES[task.priority] ?? "bg-slate-200 dark:bg-slate-700/40 text-slate-500"}`}>
                        {task.priority}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500">{task.customerMobile}</p>
                    {task.overdueAmount && (
                      <p className="text-sm font-extrabold text-rose-500 dark:text-rose-400 mt-1">
                        Overdue: {fmt.currency(task.overdueAmount)}
                      </p>
                    )}
                  </div>
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${STATUS_CLASSES[task.status] ?? "bg-slate-200 dark:bg-slate-700/40 text-slate-500"}`}>
                ...
                {/* Expanded actions */}
                {expandedId === task.id && (
                  <div className="border-t border-border dark:border-slate-700/30 px-4 pb-4 pt-3 space-y-3 bg-slate-100 dark:bg-slate-800/20">
                    {task.notes && (
                      <p className="text-xs text-foreground dark:text-slate-350 bg-slate-200 dark:bg-slate-800/40 border border-border dark:border-slate-700/50 rounded-xl p-3 font-medium">
                        {task.notes}
                      </p>
                    )}
                    {task.dueDate && (
                      <div className="flex items-center gap-1.5 text-xs text-slate-550 dark:text-slate-500 font-medium">
                    ...
                    <div className="flex gap-2 pt-1">
                      <button
                        ...
                        className="flex-1 bg-amber-500/10 hover:bg-amber-500/25 text-amber-600 dark:text-amber-400 border border-amber-500/20 rounded-xl py-2.5 text-xs font-bold flex items-center justify-center gap-1.5 transition-all"
                      >
                        <Phone size={14} />
                        Log Call
                      </button>
                      {task.status !== "resolved" && (
                        <button
                          ...
                          className="flex-1 bg-amber-500/10 hover:bg-amber-500/20 text-amber-500 border border-amber-500/20 rounded-xl py-2.5 text-xs font-bold transition-all disabled:opacity-50"
                        >
                          In Progress
                        </button>
                      )}
                      {task.status !== "resolved" && (
                        <button
                          ...
                          disabled={patchMutation.isPending}
                          className="flex-1 bg-emerald-500/10 hover:bg-emerald-500/25 text-emerald-500 dark:text-emerald-400 border border-emerald-500/20 rounded-xl py-2.5 text-xs font-bold transition-all disabled:opacity-50"
                        >
                          Resolve
                        </button>
                      )}
      ...
      {callModal && (
        <div className="fixed inset-0 z-[100] flex items-end">
          ...
          <div className="relative bg-card dark:bg-slate-900 border-t border-border dark:border-slate-800 rounded-t-3xl w-full p-6 pb-safe text-foreground dark:text-white shadow-2xl z-10">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-bold flex items-center gap-2">
                <Phone size={18} className="text-amber-500 dark:text-amber-400" /> Log Customer Call
              </h2>
              <button
            ...
            <form onSubmit={(e) => handleCall(e, callModal)} className="space-y-4">
                  ...
                  {["no_answer", "called_back", "promised"].map((o) => (
                    <button
                      ...
                      className={`py-2 rounded-xl text-xs font-bold capitalize transition-all border ${
                        callForm.outcome === o
                          ? "bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 border-amber-500/20 shadow-md shadow-amber-500/20"
                          : "bg-slate-100 dark:bg-slate-800/40 text-slate-550 dark:text-slate-400 border-border dark:border-slate-700/50 hover:text-foreground"
                      }`}
                    >
                      {o.replace(/_/g, " ")}
                    </button>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-100 dark:bg-slate-800/40 border border-border dark:border-slate-700/50 text-foreground dark:text-white placeholder:text-slate-500 rounded-xl p-3 text-sm focus:outline-none focus:border-amber-500/50 resize-none font-medium"
                />
              </div>
