...
async function main() {
  console.log("🚀 Starting Fast Bissi folder (4).xlsx Data Import...\n");
  ...
  try {
    ...
    if (ddSheet) {
      ...
      console.log(`✓ Daily Diary Accounts Imported`);
    }
    ...
    const existingCustRes = await client.query(`SELECT id, name, mobile FROM customers`);
    const custMap = new Map();
    existingCustRes.rows.forEach(c => {
      custMap.set(`${c.name.toLowerCase()}__${c.mobile}`, c.id);
    });

    const bissiSheets = [
      { name: 'Sawariya seth 5 date', schemeName: 'Sawariya Seth Bissi (5th Date)' },
      { name: 'Pyare mohan 15 date', schemeName: 'Pyare Mohan Bissi (15th Date)' },
      { name: 'Hare ka sahara bissi 20 date', schemeName: 'Hare Ka Sahara Bissi (20th Date)' },
      { name: 'Shree Krishna associate lottery', schemeName: 'Shree Krishna Associates Bissi' }
    ];
    ...
    for (const bScheme of bissiSheets) {
      ...
      console.log(`Processing Scheme "${bScheme.schemeName}"...`);
      ...
      for (let r = 1; r < rows.length; r++) {
        ...
        const dupSuffix = rawToken.replace(/^[0-9]+/, '') || '';
        const key = `${nameVal.toLowerCase()}__${mobVal || '0000000000'}`;

        let customerId = custMap.get(key);
        if (!customerId) {
          ...
          custMap.set(key, customerId);
        }

        await client.query(
          `INSERT INTO tokens (organization_id, committee_id, customer_id, raw_token_number, normalized_token_number, duplicate_suffix, status) 
           VALUES ($1, $2, $3, $4, $5, $6, 'ACTIVE')
           ON CONFLICT (committee_id, normalized_token_number, duplicate_suffix) 
           DO UPDATE SET customer_id = EXCLUDED.customer_id, raw_token_number = EXCLUDED.raw_token_number, status = 'ACTIVE'`,
          [orgId, committeeId, customerId, rawToken, normNum, dupSuffix]
        );
      }
      console.log(`✓ Scheme "${bScheme.schemeName}" Processed`);
    }
    ...
    for (const gObj of giftSheets) {
      ...
      console.log(`Processing Gift Sheet "${gObj.name}" (${monthCols.length} draw months)...`);
      ...
      for (const mCol of monthCols) {
        ...
        if (sessRes.rows.length === 0) {
          const newSess = await client.query(
            `INSERT INTO lottery_sessions (organization_id, bissi_name, lottery_date, lottery_month, notes) VALUES ($1, $2, $3, $4, 'Imported from Gift Sheet') RETURNING id`,
            [orgId, gObj.bissiName, mCol.monthLabel, mCol.monthLabel]
          );
        ...
        for (let r = 1; r < gRows.length; r++) {
          ...
          await client.query(
            `INSERT INTO lottery_gifts (session_id, token_number, customer_name, bissi_name, gift_name, status) 
             VALUES ($1, $2, $3, $4, $5, 'Collected')`,
            [sessionId, tokenNo, customerName, gObj.bissiName, giftItem]
          );
          totalGiftsSaved++;
        }
      }
    ...
    console.log(`✓ Gift Records Import Complete (${totalGiftsSaved} gifts)`);
    console.log("\n===============================================");
    console.log("🎉 ALL DATA FROM Bissi folder (4).xlsx IMPORTED SUCCESSFULLY!");
    console.log("===============================================\n");

  } catch (err) {
    console.error("❌ ERROR during fast import:", err);
  } finally {
    client.release();
    pool.end();
  }
